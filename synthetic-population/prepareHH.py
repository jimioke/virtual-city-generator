...
# print(age_column.values())

# df_gender, df_age = compute_marginals(sex_age)
# category_gender = code_category(df_gender, [' Male', ' Female'])
...
vehicle_colum = [' - No vehicle available', ' - 1 vehicle available', ' - 2 vehicles available', ' - 3 vehicles available', ' - 4 or more vehicles available']
vehicle_columns = ['Estimate; Total:' + t for t in  vehicle_colum]
df_vehicles = compute_marginals_using_columns(hhsize_vehicles, vehicle_columns)
categore_vehicle = code_category(df_vehicles, df_vehicles.columns)
print(categore_vehicle)
writeCountyFiles(df_vehicles, 'vehicles', outputFolder)

# writeCountyFiles(df_hhsize, 'hhsize', outputFolder)
