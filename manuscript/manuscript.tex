...
\begin{figure}[h!]
  \centering
  \begin{tikzpicture}[>=latex]
    \tikzset{data/.style= {fill=gray!50!, rectangle, text width=25ex, align=center} }
    \tikzset{res/.style= {draw, rectangle, rounded corners,text width=18ex,font={\small}, align=center} }
    \tikzset{exp/.style= {draw=none, rectangle,text width=18ex,font={\scriptsize}} }
    %\tikzset{head/.style= {draw=none,text width=18ex,font={\footnotesize\it}},align=center }
      
    \node[data] (aggr) at (0,0) {{\bf Aggregates (SAL)}\\{\footnotesize HH|IND|WORK|EDU}} ;
    \node[data] (samp) [right= 3cm of aggr] {{\bf Samples (SAL)}\\{\footnotesize HH|IND} } ;
    \node[res] (synhi) [below= 2cm of samp] {{\bf Synthetic Population} \\{\footnotesize HH|IND}} ;
    \node[res] (synwe) [below= 2cm of aggr] {{\bf Synthetic Population} \\{\footnotesize WORK|EDU}} ;

    \node[data, text width=15ex] (landu) [below right= of synwe] {LAND\\ USE};
    \node[res] (locwe) [below left= of landu] {{\bf TAZ Locations}\\{\footnotesize WORK|EDU}};
    \node[res] (lochi) [below right= of landu] {{\bf CELL Locations}\\{\footnotesize HH}};

    \node[res] (assig) [below right= of locwe] {{\bf Assignment}\\{\footnotesize IND$\to$WORK|EDU}};
    
    \path[draw, ->] (aggr) to [out=0,in=180] (synhi);
    \path[draw, ->] (samp) -- node[exp, right]{Hierarchical Iterative Proportional Fitting} (synhi);
    \path[draw, ->] (aggr) -- node[exp, left,align=right]{Iterative Proportional Fitting} (synwe);
    \path[draw, ->] (synwe) to [out=270,in=180]   (landu);
    \path[draw, ->] (synhi) to [out=270,in=0]   (landu);
    \path[draw, ->] (landu) to [out=195,in=0]   (locwe);
    \path[draw, ->] (landu) to [out=-15,in=180]   (lochi);
    \path[draw, ->] (lochi) to [out=270,in=0]   (assig);
    \path[draw, <-] (locwe) to [out=270,in=180] (assig);

    
    \node[exp,align=center] (exp-assig) [above=.25cm of assig] {Entropy Maximization Model};
    \node[exp,align=center] (exp-landu) [below=.25cm of landu] {rasterization \& weighting};
    %\node[exp] (taz-source) [right= of synhi] {Hierarchical Iterative Proportional Fitting};
    %\node[exp] (cell-source)[left= of synhi]{Generated};
...
\begin{figure}[h!]
  \centering
  \begin{tikzpicture}[>=latex]
    \tikzset{pro/.style= {fill=gray!50!, rectangle, text width=30ex, align=center} }
    \tikzset{res/.style= {draw, rectangle, rounded corners,text width=22ex,font={\small}, align=center} }
    \tikzset{exp/.style= {draw=none, rectangle,text width=18ex,font={\scriptsize}} }
    %\tikzset{head/.style= {draw=none,text width=18ex,font={\footnotesize\it}},align=center }
      
    \node[res] (ff) at (0,0) {network freeflow parameters} ;
    \node[pro] (trip) [below= of ff] {{\bf Trip Distribution}\\{\footnotesize static traffic assignment} } ;
    \node[res] (tt) [below= of trip] {congested travel times} ;
    \node[res] (est) [below= of tt] {peak \& off-peak network cost estimates} ;

    % \node[pro] (pre) [below= 2cm of est] {run Pre-Day} ;
    \node[pro] (calib) [below = of est] {{\bf Calibration \& Validation}\\{\footnotesize day-to-day learning}};
    \node[res] (fin) [below = of calib] {final {\bf PreDay} and {\bf WithinDay} parameters};
    
    \path[draw, ->] (ff) --(trip);
    \path[draw, ->] (trip) -- (tt);
    \path[draw, ->] (tt) -- (est);
    \path[draw, ->] (est) to (calib);
    %path[draw, ->] (pre) to [out=270,in=0]   (calib);
    \path[draw, ->] (calib) to  (fin);
    % \path[draw, ->] (lochi) to [out=270,in=0]   (assig);
    % \path[draw, <-] (locwe) to [out=270,in=180] (assig);
  ...
  \end{tikzpicture}
  \caption{Demand piece of the prototype city generation framework}
  \label{fig:framework}
\end{figure}
