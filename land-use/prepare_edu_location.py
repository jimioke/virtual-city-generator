...
# PROJECTION = {'proj': 'lcc', 'lat_1': 38.3, 'lat_2': 39.45, 'lat_0': 37.66666666666666, 'lon_0': -77, 'x_0': 400000, 'y_0': 0, 'datum': 'NAD83', 'units': 'm', 'no_defs': True}
...
def get_k_thru_12():
    ...
    df.to_csv(outFolder+'k_thru_12_all.csv')
...
def get_higher_edu():
    ...
    df = df.loc[df['county'].isin(counties)]
    df.to_csv(outFolder+'high_edu_all.csv')
    geometry = [Point(xy) for xy in zip(df.X, df.Y)]
    df = df.drop(['X', 'Y'], axis=1)
    crs = {'init': 'epsg:4326'}
    gdf = gpd.GeoDataFrame(df, crs=crs, geometry=geometry)
