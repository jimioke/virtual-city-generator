...
def constructNodesLinks(G, originalG, tempnodeDict):
    ...
    for upnode, dnnode, data in G.edges(data=True):
        ...
        link = Link(link_id, 1, 1, upnode, dnnode, name)
    ...
    for nodeId, data in G.nodes(data=True):
        ...
        if nodeType == 2:
            ...
            in_edges = [((u,v, 0), attr(data, 'width'), attr(data, 'lanes')) for u,v,data in G.in_edges(nodeId, data=True)]
            out_edges = [((u,v, 0), attr(data, 'width'), attr(data, 'lanes')) for u,v,data in G.out_edges(nodeId, data=True)]
            # print([ (edge[1], edge[2]) for edge in in_edges + out_edges])
...
def attr(data, attr):
    if attr in data:
        if attr == 'width' or attr == 'lanes':
            try:
                print(type(data[attr]), data[attr])
