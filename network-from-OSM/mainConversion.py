...
inputFolder = "Inputs/GBA_Full_RB_05June"
#inputFolder = "Inputs/Kat"
baseFname = os.path.join(inputFolder,"Base.csv")
baseAttFname = os.path.join(inputFolder,"Base-attributes2.csv")
...
def main():
    ...
    connSumo = checkConnections(connSumo,baseAttributes,tempnodeDict,lanes)    
    laneconnections,turninggroups = constructConnections(connSumo,nodes,links,segments,lanes,segToLink)
    
    print "----------Constructed connections-------------"
    ...
    net.write(foldername='Outputs/Output'+inputFolder.split('/')[1])
